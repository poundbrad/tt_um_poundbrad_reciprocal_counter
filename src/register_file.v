...
module register_file (
    ...
    // Channel 0 and 1 configuration output
    output reg  [31:0] ch0_gate_cycles,
    output reg  [31:0] ch1_gate_cycles,
    output reg  [31:0] ch0_timeout_refcount,
    output reg  [31:0] ch1_timeout_refcount,

    input wire  [31:0] ch0_count,
    input wire [31:0] ch1_count,
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ch0_gate_cycles         <= 32'h0000_0000;
            ch1_gate_cycles         <= 32'h0000_0000;
            ch0_timeout_refcount    <= 32'h0000_0000;
            ch1_timeout_refcount    <= 32'h0000_0000;
            ...
            // These execute on every clock, even when reg_write is zero.
                ch0_clear_timeout_latched    <= 1'b0;
                ch1_clear_timeout_latched    <= 1'b0;
                ch0_clear_overflow_latched   <= 1'b0;
                ch1_clear_overflow_latched   <= 1'b0;
            ...
            if (reg_write) begin
                case (reg_addr)
                    ADDR_CH0_GATE_CYCLES: begin
                        ch0_gate_cycles <= reg_wdata;
                    end

                    ADDR_CH1_GATE_CYCLES: begin
                        ch1_gate_cycles <= reg_wdata;
                    end

                    ADDR_CH0_TIMEOUT_REFCOUNT: begin
                        ch0_timeout_refcount <= reg_wdata;
                    end

                    ADDR_CH1_TIMEOUT_REFCOUNT: begin
                        ch1_timeout_refcount <= reg_wdata;
                    end
    ...
    always @(*) begin
        ...
        case (reg_addr)
            ADDR_CH0_GATE_CYCLES: begin
                reg_rdata = ch0_gate_cycles;
            end

            ADDR_CH1_GATE_CYCLES: begin
                reg_rdata = ch1_gate_cycles;
            end

            ADDR_CH0_TIMEOUT_REFCOUNT: begin
                reg_rdata = ch0_timeout_refcount;
            end

            ADDR_CH1_TIMEOUT_REFCOUNT: begin
                reg_rdata = ch1_timeout_refcount;
            end
